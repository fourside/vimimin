...
import type { BookmarkListResponse, FinderEntry } from "../shared/messages.js";
import {
  removeFinder,
  showFinder,
  updateFinderList,
...
export class BookmarkFinderSession {
  ...
  async start(onClose: () => void): Promise<void> {
    ...
    if (
      ...
      response !== null &&
      Array.isArray((response as Record<string, unknown>).bookmarks)
    ) {
      this.allItems = (response as BookmarkListResponse).bookmarks;
    }
    this.filtered = this.allItems;

    showFinder("Search bookmarks...", {
